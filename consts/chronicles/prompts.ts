...
export const STORY_PROMPT_SYSTEM_MESSAGE = PromptTemplate.fromTemplate(`
	...
	Story:
	The player is playing as Guy Fawkes. The player begins in a dimply lit room with a handful of men, the leader explaining their plot to them. The player is to blow up the Houses of Parliament with gunpowder. The players role in the plan is to aquire gunpowder and firewood and transport it into the Houses of Parliment undetected. The player needs 36 barrels of gunpowder in total that need to be stolen from a military base. Once the player has stolen the gunpowder they then need to find a way to transport the gunpowder into the Houses of Parliment through the secret tunnel undetected. Once inside the basement of the Houses of parliment the player is caught by the guards, the player must convinve the guards that they are not doing anything suspicious. Once the player has convinced the guards he is not doing anything he must then race against the clock to light the gunpowder before the guards catch him. The player must then escape the Houses of Parliment before the gunpowder explodes. Once the explosion has happened the player must escape the city of London to Spain where he will be safe. Once the player has escaped the city of London the game ends.
