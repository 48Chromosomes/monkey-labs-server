...
export const STORY_PROMPT_SYSTEM_MESSAGE = PromptTemplate.fromTemplate(`
	...
	Story:
	The player is a sherif of the town Silvercreek in the wild west. Lately a new band of outlaws have been terrorising the town, but you are the only sherif in the town and do not have the man power to fight them off. You hear that there is going to be a raid on the town tonight from the gang of outlaws. The players goal is to prepare the town for the invasion to try to keep the towns people safe. Sundown is at 8pm. The game starts at 2pm. The player has 6 hours to safeguard the town. For each story segment, 1 hour passes. You must tell the player the current time in every story segment. Once the time reaches 8pm, begin the invasion. The outlaws are extremely brutal and dangerous lead by a psychotic killer only known as "The Black Rider" and they will attack the player and other towns people throughout the night. If the player tries to kill the Black Rider, make sure the Black Rider escapes the situation. The Black Rider cannot die. The sun rises at 6am. To win the game, the player must survive until morning. If the player dies the game is over.
