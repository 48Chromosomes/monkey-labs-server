...
export const GENERATE_CHARACTER =
	'Generate a Dungeons & Dragons character style. The chartacter is a super-hero called Snow Roach who is a true neutral who wears a white costume with a helmet shaped like a cockroachs head, his background is "social justice", his equipment contains an Ice-ray blaster, a camoflage cloak, and an equity shield, he is proficient in hand-to-hand combat and scuttling up walls. Your response should be valid JSON format for the following type: { alignment: string; background: string; class: string; equipment: string[]; name: string; sex: string; proficiencies: string[]; race: string; stats: { charisma: number; constitution: number; dexterity: number; intelligence: number; strength: number; wisdom: number; }, appearance: { hair_color: string; age: string; body_type: string; clothing: string; }}. Do not use newlines or /n in your response.';
...
export const STORY_PROMPT_SYSTEM_MESSAGE = PromptTemplate.fromTemplate(`
	...
	Story:
	The player is playing as a super hero named Snow Roach. His mission is to restore order and bring justice to the crab army, even if this means violence. The Snow Roach got his powers from a freak accident in a lab where his creator was working on an experimental serum designed to enhance human abilities, but a poor decision led to a mishap, creating Snow Roach. His arch nemesis, Crab-man, is in an underwater lair and is terrorising the city with his army of crabs. His crabs march through the streets towards city hall, rioting as they go. They want to overthrow the Lobster King who runs the city as they believe he wants to externminate all the crabs. The player must first fight off the crab army as it attacks city hall. Once city hall is safe the player learns that a local kindergarden is being attacked, the player must defend the kindergarden. Once the kindergarden is safe the player must then track down the Crab-mans hidden underwater layer and defeat him so the armies will stop. Once the Snow Roach has found the lair and defeated Crabman, the story is over.
