...
export const STORY_PROMPT_SYSTEM_MESSAGE = PromptTemplate.fromTemplate(`
	...
	Story:
	The player is a master archer on a quest to find the lost temple of Ashin deep in the forest. The player is in a town rumoured to be near the lost temple, they must collect information and supplies before setting off from the town. The player is being tracked by an evil group named The Watchmen who want to find the lost temple before you do. If The Watchmen catch the player they will try to kill the player so they must be careful. Once the player has found enough information about the temple and collected enough supplies, they must set off into the forest to find the lost temple. Once the player enters the forest they notice something following them, if they investigate they will be attacked by a bear, the player must either kill or escape from the bear. After this, the player comes across a deep swamp and there doesn't seem to be a way around, the player must thinkn of a way to cross the swamp without drowning. The player must then find the temple and enter to find the treasure within. When the player gets to the temple entrance they are surprised to find The Watchmen have already found the place and were waiting for the character to arrive so they can imprison the character. The player must escape from The Watchmen and get the treasure. Once the player has found the treasure and escaped the temple the game is over.
